...
// 	rawImageType_8bit::Pointer montage_gfp;
	...
	if( onlyTrace == 0 )
	{
		...
		{
			numThreadsRows = 10;
			numThreadsRows_split = 10;
			numThreadsCols = 8;
			numThreadsCols_split = 4;
		}
		else
		{
			numThreadsRows = 1;
...
// 		gfpImageType::Pointer gfpLog_1;
		...
		for (unsigned int i = 0; i < 6; ++i)
		{
			...
			std::cout<<std::endl<<"Done scale: "<<i;
		}
	...
	if( onlyTrace == 3 )
	{
		...
		std::cout<<std::endl<<"NEWNEW";
		rawImageType_16bit::Pointer montage_gfp = readAndRescale_16to16(argv[2]);
		gfpImageType::Pointer img_trace = preprocessingMNT(montage_gfp);
		std::string tempFileName_4 = gfpFileNameNoExt + "_PREP_MNT.mhd";
                writeImage<gfpImageType>(img_trace,tempFileName_4.c_str());

		gfpImageType::Pointer img_trace_2 = readerINcasterOUT< rawImageType_16bit,gfpImageType >(argv[2]);
		MultipleNeuronTracer * MNT = new MultipleNeuronTracer();
		MNT->LoadCurvImage_2(img_trace_2);
		bool runLogOut = false;
		MNT->setFlagOutLog(runLogOut);
		MNT->runNDX();
		gfpImageType::Pointer out = MNT->getNDX();
		
		std::string tempFileName = gfpFileNameNoExt + "_NDX.mhd";
		writeImage<gfpImageType>(out,tempFileName.c_str());
// 		MNT->LoadCurvImage_2(img_trace);
// 		MNT->ReadStartPoints_1(soma_Table, 0);
// 		MNT->SetCostThreshold(400);
...
// 			desiredRegionBigTileMON.SetIndex(initialBigIndexCEN);
			...
			gfpImageType::Pointer NDXDesiredRegion;
...
// 			
				...
				if( i ==2 )
				{
					std::string NDXName = temp + "_NDX.mhd";
					NDXDesiredRegion = readImageRegion< gfpImageType >( NDXName.c_str(), desiredRegionBigTileLOG );
 					std::string tempFileName_45 = filePath + "/BigTile_" + srr + "_NDX.mhd";
 					writeImage< gfpImageType >(NDXDesiredRegion,tempFileName_45.c_str());
				}
...
// 
		...
		#pragma omp critical
				...
				gfpROIFilterType::Pointer ROIfilter9 = gfpROIFilterType::New();
				ROIfilter9->SetRegionOfInterest(desiredRegion2_float);
				ROIfilter9->SetInput(NDXDesiredRegion);
		#pragma omp critical
				ROIfilter9->Update();
				gfpDuplicatorType::Pointer floatDuplicator9 = gfpDuplicatorType::New();
				floatDuplicator9->SetInputImage(ROIfilter9->GetOutput());
		#pragma omp critical
				floatDuplicator9->Update();
				gfpImageType::Pointer NDXImage = floatDuplicator9->GetOutput();
...
// 				MNT->setLogScale(LoGDesiredRegion[5],5);
				...
				MNT->setFlagPipeline( flagPipeline );
