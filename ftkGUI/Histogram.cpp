...
Histogram::Histogram(std::string filename, int numBins, QWidget * parent)
: QWidget(parent)
{
		numofbins=0;
		...
		this->ReadHistogramData(filename.c_str());
		this->Normalize();
		this->SetNumofBins(numBins);
		this->findFrequencies();		
		this->setBucketNames();	
		this->ConstructBarChart();
}

bool Histogram::ReadHistogramData(const char* fileName)
{  
...
void Histogram::ConstructBarChart() 
{
	if (status) 
	{
		...
		/*
		frequencies = vtkSmartPointer<vtkIntArray>::New();
		...
		*/
		...
		layout->addWidget(chartView->GetWidget());
...
int main(int argc, char* argv[])
{
	...
	if (! his->ReadHistogramData(argv[1])) {
		...
		his->findFrequencies();		
		his->setBucketNames();	
		his->ConstructBarChart(argc, argv);		
	}
	return 0;
}
*/
