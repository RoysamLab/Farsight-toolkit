...
subplot(2,2,2);
hA = imshow(reshape(img(:,:,z,:),[size(img,1) size(img,2) 3]),'InitialMagnification',100);
...
subplot(2,2,1);
hB = imshow(reshape(img(:,y,:,:),[size(img,1) size(img,3) 3]),'InitialMagnification',100);
...
subplot(2,2,3);
hC = imshow(reshape(img(x,:,:,:),[size(img,2) size(img,3) 3]),'InitialMagnification',100);
...
set(findobj(gcf,'Tag','Cinfo'),'String',sprintf('Value: (%.2f,%.2f,%.2f)',img(hx,hy,hz,1),img(hx,hy,hz,2),img(hx,hy,hz,3)));
...
set(hB,'CData',reshape(img(:,round(px),:,:),[size(img,1) size(img,3) 3]));
subplot(2,2,3);
...
set(hC,'CData',reshape(img(round(py),:,:,:),[size(img,2) size(img,3) 3]));
...
set(hA,'CData',reshape(img(:,:,round(px),:),[size(img,1) size(img,2) 3]));
subplot(2,2,3);
...
subplot(2,2,1);
hB = findobj(gcf,'Tag','imageB');
set(hB,'CData',reshape(img(:,round(py),:,:),[size(img,1) size(img,3) 3]));
