...
function showslice(img,showfun);
    ...
    switch selection,
        case 3,
            showfun(reshape(img(:,:,idxpos,:),[size(img,1) size(img,2) 3]));
            set(cinfo,'String',sprintf('%i/%i',idxpos,size(img,3)));    
        case 2,
            showfun(reshape(img(:,idxpos,:,:),[size(img,1) size(img,3) 3]));
            set(cinfo,'String',sprintf('%i/%i',idxpos,size(img,2)));    
        case 1,
            showfun(reshape(img(idxpos,:,:,:),[size(img,2) size(img,3) 3]));
