...
sig = 0.2;
%sig = 0.2;
Q = computevec([0 1],M,M,sig,sig*0.3); quiver(real(Q),imag(Q))
...
for l = 1:L,
    for k = -1:1,
     ...
     % Zproj(:,l,k+2) = Zproj(:,l,k+2)  - 1/sqrt(l*(l+1))* LegExpansionW(:,l+k+1)*ClebschGordan(l,l+k,1,0,1,1);
       Zproj(:,l,k+2) = LegExpansionZ(:,l+k+1)*ClebschGordan(l,l+k,1,0,0,0) - 1/sqrt(l*(l+1))*LegExpansionW(:,l+k+1)*ClebschGordan(l,l+k,1,0,1,1)*exp(i*0);
...
for l = 1:1:(L-1),
    for k = [-1 0 1 ],
          recon3d = recon3d + STmultRealAsym(TBasis{l,k+2},SHtest{l+1}(:,25,25,45),1);
    end;
end;
...
recon3d =  STmultRealAsym(SHtest{l+1},A,1);

rad = 9;
recon3dZa = real(reshape(recon3d(2,:,:,:),[CS CS CS])) .* (D < rad & D > rad-1);
recon3dXa = -real(reshape(recon3d(1,:,:,:),[CS CS CS]))*sqrt(2)  .* (D < rad & D > rad-1 );
recon3dYa = -imag(reshape(recon3d(1,:,:,:),[CS CS CS]))*sqrt(2)  .* (D < rad & D > rad-1 );
...
hc = coneplot(X,Y,Z,-recon3dX,-recon3dY,-recon3dZ,0.03,'nointerp');
...
%hold on;
%rad= 8.5;
%hs = surf(Q*rad,W*rad,R*rad);
%set(hs,'FaceColor','blue','EdgeColor','none');
