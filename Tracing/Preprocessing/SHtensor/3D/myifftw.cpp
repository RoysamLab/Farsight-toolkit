...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
   ...
   if (mxGetClassID(Img) == mxSINGLE_CLASS)
   {
	...
	fftwf_complex *ro = (fftwf_complex*) mxGetData(plhs[0]);	
        ...
        report_timing("\n init, float ");	
#endif
	const fftwf_plan plan = fftwf_plan_guru_dft(rank,(const fftwf_iodim*) dims, howmany_rank, (const fftwf_iodim*)howmany_dims,ro,ro, -1, flags | FFTW_PRESERVE_INPUT);
   ...
   if (mxGetClassID(Img) == mxDOUBLE_CLASS)
   {
	...
	fftw_complex *ro = (fftw_complex*) mxGetData(plhs[0]);	
        ...
        report_timing("\n init, double ");	
#endif
	const fftw_plan plan = fftw_plan_guru_dft(rank,(const fftwf_iodim*) dims, howmany_rank, (const fftwf_iodim*)howmany_dims,ro,ro, -1, flags);
