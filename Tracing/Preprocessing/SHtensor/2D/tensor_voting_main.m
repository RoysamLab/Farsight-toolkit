...
%  figure, imshow(uint8(128*ballness));
 figure(2), imagesc(orientation*180/pi.*(stickness>0.5*10^4));
...
figure(3)
subplot(2,2,1); imagesc(orientation*180/pi.*(stickness>0.5*10^4));
subplot(2,2,2); imagesc(gradmag);
subplot(2,2,3); imagesc(sticknew)
subplot(2,2,4); imagesc(orientnew*180/pi.*(stickness>0.5*10^4));
