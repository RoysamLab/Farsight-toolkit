...
vnl_matrix< std::complex <float> > myconvolve( vnl_matrix< std::complex<float> > &mat1, vnl_matrix< std::complex <float> > &mat2)
{
	...
	fftwf_plan plan = fftwf_plan_dft_2d(mat1.rows(),mat1.cols(),in, out,FFTW_FORWARD,FFTW_MEASURE);
...
float order;
	...
	if(argc < 2 || argc > 5)
	{
		printf("Usage : scalar_voting_2d.exe gradmag_file dircos_file dirsin_file [options_file]");
		return -1;
	}
	//assert(argc==3);
	//get options
	...
	vnl_matrix < float> U0(rows,cols,0);
	int pc = 0;
	...
	for(int co = 0; co <= 4*order-2; co = co + 2)
	{
		...
		for(;!iter1.IsAtEnd();++iter1,++iter2)
		{
			...
			cvals[index[1]][index[0]] = iter1.Get()*Cpx(cos(-co*iter2.Get()),sin(-co*iter2.Get()));
		}
		...
		for(int xco = K; xco < rows + K ; xco++)
		{
			...
			{
				U0[xco-K][yco-K] += a[pc] * Utemp[xco][yco].real();
			}
		}
	...
	std::cout <<setprecision(2)<<U0.extract(10,10);
	float min1 = 1e10f;
	float max1 = -1;
	...
	for(int xco = 0; xco < rows ; xco++)
	{
		...
		{
			min1 = MIN(U0[xco][yco],min1);
			max1 = MAX(U0[xco][yco],max1);
	...
	for(;!outiter.IsAtEnd();++outiter)
	{
		...
		outiter.Set(MAX(MIN((U0[index[1]][index[0]]*rescale_factor),255),0));
	}
	argv[1][strlen(argv[1])-4] = 0;
	char buffer[1024];
	sprintf(buffer,"%s_SV2D.tif",argv[1]);
	writeImage<Input2DImageType>(out,buffer);
	return 0;
}
