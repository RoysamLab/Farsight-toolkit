...
% filename = 'C:\Users\arun\Research\TensorVoting\SHtensor\Diadem\CF_processed_cropped.tif';
...
for outer_counter = 1:size(imf,1)
    ['counter = ' int2str(outer_counter)]
    
im_temp = imread(filename,outer_counter);
...
[X_rows, X_cols, F_rows, F_cols, N_rows, N_cols] = fdct_wrapping_param(C,N,M);
...
%         end
        if j ~= 1 
            ...
            Ccos{j}{l} = cos(2*mod(theta,pi))*C{j}{l}; %TODO :Fix me
%             Ccos{j}{l}  = C{j}{l};
            Csin{j}{l} = sin(2*mod(theta,pi))*C{j}{l};
        else
            Ccos{j}{l} = 0*C{j}{l};
            Csin{j}{l} = 0*C{j}{l};
        end
    ...
    temp_restored = real(ifdct_wrapping(C,is_real,N,M));
    temp_restored_cos = real(ifdct_wrapping(Ccos,is_real,N,M));
    temp_restored_sin = real(ifdct_wrapping(Csin,is_real,N,M));
    % L1 iterations
    ...
    for nupdate = 1:nell1
      ...
      temp_update = real(ifdct_wrapping(D,is_real,M,N));
...
restored_img = (restored_img(1:size(im_temp,1),1:size(im_temp,2)));

temp_restored_cos = temp_restored_cos(1:size(im_temp,1),1:size(im_temp,2));
temp_restored_sin = temp_restored_sin(1:size(im_temp,1),1:size(im_temp,2));
sum1 = sqrt(temp_restored_cos.^2 + temp_restored_sin.^2);
temp_restored_cos = temp_restored_cos./sum1;
temp_restored_sin = temp_restored_sin./sum1;
temp_restored_cos = sign(temp_restored_sin).*sqrt(1+temp_restored_cos)/sqrt(2);
temp_restored_sin = sqrt(1-temp_restored_cos.^2);
