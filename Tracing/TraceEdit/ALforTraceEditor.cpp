...
#include "ALforTraceEditor.h"

ALforTraceEd::ALforTraceEd(bool val)
{	
	pixel_class = val;
...
//void ALforTraceEd::SetLabelView(LabelImageViewQT *view)
//{
//	labelView = view;
//	if(pixel_class)
//	{
...
void ALforTraceEd::RunALClassification(bool val)
{	
	from_model = val;
	if(!from_model)
	{
...
void ALforTraceEd::Start_Training(vtkSmartPointer<vtkTable> pTable)
{
...
void ALforTraceEd::ALDialogPopUP(bool first_pop, std::vector<std::pair<int,int> > query_labels)
{
	...
	if(!first_pop)		
	{
		for(int i=0; i<active_queries.size(); ++i)
		{
			...
			{	
				...
				this->show();
				alDialog =  new ALDialog(active_queries.size(), mclr->test_table, mclr->numberOfClasses, active_queries, mclr->top_features);	
	...
	alDialog =  new ALDialog(active_queries.size(), mclr->test_table, mclr->numberOfClasses, active_queries, mclr->top_features);
	ALDialog->setWindowTitle(QString("Active Learning Window: Specify Class for Cell %1").arg(active_queries[0]));
	emit zoomINtoCell(active_queries[0]);
	connect(alDialog, SIGNAL(retrain(bool, std::vector<std::pair<int,int> >)), this, SLOT(ALDialogPopUP(bool, std::vector<std::pair<int,int> >)));
	connect(alDialog, SIGNAL(start_classification(bool)), this, SLOT(Start_Classification(bool)));
	alDialog->show();
...
void ALforTraceEd::Start_Classification(bool create_model)
{
	if(create_model)
	{
...
void ALforTraceEd::Perform_Classification()
{
