...
//Constructors:
ALDialog::ALDialog(int num_queries, vtkSmartPointer<vtkTable> table,int num_classes,std::vector<int> active_query,std::vector<int>top_feats,QWidget *parent)
: QDialog(parent)
{
	rejectFlag = false;
	queries = active_query;
	num_of_classes = num_classes;
	query_label.resize(num_queries);
	buttongroup.resize(num_queries);
	...
	{
	std::vector<QHBoxLayout *> rows  = Sample_Details(table,num_classes,active_query[i],top_feats,i);
	...
	layout->addLayout(finalRow,2*num_queries+2,0,0);
...
//Constructors:
ALDialog::ALDialog(/*std::string validate,*/int num_queries, 
...
: QDialog(parent)
{
	...
	for(int i=0;i<num_queries;++i)
	{
		std::vector<QHBoxLayout *> rows  = Validation_Sample_Details(table,classval,rowvals[i],i,num_classes);
...
std::vector<QHBoxLayout *> ALDialog::Sample_Details(vtkSmartPointer<vtkTable> table,int num_classes,int active_query,std::vector<int>top_feats,int group)
{
...
std::vector<QHBoxLayout *> ALDialog::Validation_Sample_Details( vtkSmartPointer<vtkTable> table,int classval,int PIA_query,int group,int num_classes)
{	
...
void ALDialog::StartClassification()
{
	for(int i=0; i<query_label.size(); ++i)
	{
...
void ALDialog::Retrain()
{
	emit retrain(false, query_label);
	this->accept();
}

void ALDialog::finished()
{
	finish = false;
...
void ALDialog::Set_Class_Validate()
{
	for(int i =0; i< button_vector.size() ; ++i)
	{
		QRadioButton * test_button = button_vector.at(i);
...
void ALDialog::User_Validation_Response()
{
	for(int i =0; i< button_vector.size() ; ++i)
	{
		QRadioButton * test_button = button_vector.at(i);
