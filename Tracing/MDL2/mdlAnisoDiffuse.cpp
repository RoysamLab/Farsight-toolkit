...
#include "mdlAnisoDiffuse.h"

namespace mdl
{

//Constructor
AnisoDiffuse::AnisoDiffuse()
{
	debug = false;
	overwriteInput = true;
	useIso = false;
...
void AnisoDiffuse::SetInput(ImageType::Pointer inImage)
{
	m_inputImage = inImage;
}

bool AnisoDiffuse::Update()
{
	if(!m_inputImage)
		return false;
	...
	if(overwriteInput)
	{
		itk::ImageRegionIterator< ImageType > itr( m_inputImage, m_inputImage->GetBufferedRegion() );
		itk::ImageRegionIterator< ImageType > itr2( m_outputImage, m_outputImage->GetBufferedRegion() );
		for(itr.GoToBegin(), itr2.GoToBegin() ; !itr.IsAtEnd(); ++itr, ++itr2)
		{
			itr.Set( itr2.Get() );
		}
		m_outputImage = 0;
...
ImageType::Pointer AnisoDiffuse::GetOutput()
{
	return m_outputImage;
}

bool AnisoDiffuse::DoMDLAniso()
{
	...
	PixelType * input = m_inputImage->GetBufferPointer();
	...
	while (timesDiffuse > 0 )
	{
		...
		if(!useIso)	//Do anisotropic diffusion
		{
			for (int k=border; k<sizeZ-border; k++) 
			{
				for (int j=border; j<sizeY-border; j++) 
