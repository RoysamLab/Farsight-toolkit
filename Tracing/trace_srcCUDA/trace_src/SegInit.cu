...
inline double vnl_math_max(double x, DOUBLE y) { return (x > y) ? x : y; }
inline double vnl_math_min(DOUBLE x, double y) { return (x < y) ? x : y; }
...
__device__
void cuda_matrix_point(DOUBLE *x, DOUBLE *y, DOUBLE *z, cTRMatrix *Result )
{
   cTVertex pt;

   pt.p[0] = Result->r[0][0]*(*x) + Result->r[0][1]*(*y) + Result->r[0][2]*(*z);
   pt.p[1] = Result->r[1][0]*(*x) + Result->r[1][1]*(*y) + Result->r[1][2]*(*z);
   pt.p[2] = Result->r[2][0]*(*x) + Result->r[2][1]*(*y) + Result->r[2][2]*(*z);
...
__device__
float cuda_partial_rho_e1(double x, double y, double z, double e1, double e2 )
{
...
__global__
void update_e1_kernel(cTVertex *gVertexU, float a1, float a2, float a3, float e1, cTFacet *pFacets, int Num_facets, float *pF, cTRMatrix *R, float dt, float *sde1)
{

   int globalId = blockIdx.x * 64 + threadIdx.x;
   if(globalId >= Num_facets) {
      return;
   }
    ...
    float ss[3];
     ...
     x = gVertexU[pFacets[globalId].vertex[0]].p[0];
     y = gVertexU[pFacets[globalId].vertex[0]].p[1];
     z = gVertexU[pFacets[globalId].vertex[0]].p[2];

     de1 = cuda_partial_rho_e1(x,y,z,e1,1.0);
     ss[0] = a1*x*de1;
     ss[1] = a2*y*de1;
     ss[2] = a3*z*de1;

     x = gVertexU[pFacets[globalId].vertex[1]].p[0];
     y = gVertexU[pFacets[globalId].vertex[1]].p[1];
     z = gVertexU[pFacets[globalId].vertex[1]].p[2];
     ...
     x = gVertexU[pFacets[globalId].vertex[2]].p[0];
     y = gVertexU[pFacets[globalId].vertex[2]].p[1];
     z = gVertexU[pFacets[globalId].vertex[2]].p[2];
     ...
     cuda_matrix_point(&ss[0],&ss[1],&ss[2], R);

     sde1[globalId] = dt/2.0*pF[globalId]*(ss[0]*pFacets[globalId].normal[0]+ss[1]*pFacets[globalId].normal[1]+ss[2]*pFacets[globalId].normal[2]);

}


//update shape parameter using coordinate descent
void cuda_update_e1(void *cpp_vessel,void *cpp_pFacets, int Num_facets, DOUBLE* pF, void *cpp_R, void *cpp_damp, void *cpp_gVertexU)
{
   ...
   sde1 = (float*)malloc(sizeof(float)*Num_facets);
   ...
   /*
   printf("gVertexU->p[2] = %f\n", gVertexU->p[2]);
   printf("pFacets[0]->normal[1] = %f\n", pFacets[0].normal[1]);
   printf("R[2][1] = %f\n", R->r[2][1]);
   printf("pF[4] = %f\n", pF[4]);
   */

   /* these all allocate too much space, but we'll only transfer across
      what we need each time */
   cudaMalloc((void**)&d_gVertexU, sizeof(cTVertex)*6000);
   cudaMalloc((void**)&d_pFacets, sizeof(cTFacet)*Num_facets);
   cudaMalloc((void**)&d_pF, sizeof(float)*Num_facets);
   cudaMalloc((void**)&d_R, sizeof(cTRMatrix));
   cudaMalloc((void**)&d_sde1, sizeof(float)*Num_facets);

   cudaMemcpy(d_gVertexU, gVertexU, sizeof(cTVertex)*6000, cudaMemcpyHostToDevice); // should this be num_facets?
   cudaMemcpy(d_pFacets, pFacets, sizeof(cTFacet)*Num_facets, cudaMemcpyHostToDevice);
   cudaMemcpy(d_pF, pF, sizeof(float)*Num_facets, cudaMemcpyHostToDevice);
   cudaMemcpy(d_R, R, sizeof(cTRMatrix), cudaMemcpyHostToDevice);

   int numBlocks = Num_facets/64;
   if((Num_facets)%64 != 0) {
      numBlocks++;
   }
   ...
   update_e1_kernel <<< blocks, threads >>> (d_gVertexU, vessel->a1, vessel->a2, vessel->a3, vessel->e1, d_pFacets, Num_facets, d_pF, d_R, damp->dt, d_sde1);

   cudaMemcpy(sde1, d_sde1, sizeof(float)*Num_facets, cudaMemcpyDeviceToHost);
...
//    printf("total_sde1 = %f\n", total_sde1);
    if(total_sde1+vessel->e1 > 1.0)
        total_sde1 = 0;
//    printf("total_sde1 after = %f\n", total_sde1);

    vessel->e1 += total_sde1;
