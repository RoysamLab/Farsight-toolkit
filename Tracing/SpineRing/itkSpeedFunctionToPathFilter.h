...
public:
  ...
  InputImagePointer GetArrivalFunc()
  {	
	  InputImageType::IndexType arrIndex;
		for (int ii=0; ii<3; ii++)
			arrIndex[ii] = m_Info[0].GetEndPoint()[ii];
		std::cout << "End Arrival Val=" << m_CurrentArrivalFunction->GetPixel(arrIndex) << std::endl;
		for (int ii=0; ii<3; ii++)
			arrIndex[ii] = m_Info[0].GetStartPoint()[ii];
		std::cout << "start Arrival Val=" << m_CurrentArrivalFunction->GetPixel(arrIndex) << std::endl;
		for (int ii=0; ii<3; ii++)
			arrIndex[ii] = m_Info[0].GetStartPoint()[ii]+1;
		std::cout << "start+1 Arrival Val=" << m_CurrentArrivalFunction->GetPixel(arrIndex) << std::endl;
