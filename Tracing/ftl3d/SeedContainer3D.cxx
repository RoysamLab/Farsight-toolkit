...
void SeedContainer3D::Detect3Dseeds(ImageType3D::Pointer image) {
  ...
  for (glndx[2] = 0;
       (unsigned int)glndx[2] < sz[2] - this->GridSpacing;
       glndx[2] += GridSpacing)
    {
    for (glndx[1] = 0;
         (unsigned int)glndx[1] < sz[1] - this->GridSpacing;
         glndx[1] += GridSpacing)
      {
      for (glndx[0] = 0;
          (unsigned int)glndx[0] < sz[0] - this->GridSpacing;
          glndx[0] += GridSpacing)
        {
        ImageType3D::IndexType ndx = glndx;
        ImageType3D::IndexType endx;
        endx[0] = glndx[0] + GridSpacing; endx[0] = ((unsigned int)endx[0]>sz[0]) ? sz[0] : endx[0];
        endx[1] = glndx[1] + GridSpacing; endx[1] = ((unsigned int)endx[1]>sz[1]) ? sz[1] : endx[1];
        endx[2] = glndx[2] + GridSpacing; endx[2] = ((unsigned int)endx[2]>sz[2]) ? sz[2] : endx[2];
...
void SeedContainer3D::Detect2Dseeds(ImageType2D::Pointer image)
{
  ...
  for ( p[0] = 0; (unsigned int)p[0] < size[0]; p[0]+=GridSpacing)
    {
    for ( p[1] = 0; (unsigned int)p[1] < size[1]; p[1]++)
      {
  ...
  for ( p[1] = 0; (unsigned int)p[1] <size[1] ; p[1]+=GridSpacing)
    {
    for ( p[0] = 0; (unsigned int)p[0] <size[0] ; p[0]++)
      {
      val = image->GetPixel(p);
      if(val < m)
        {
