...
void EigenAnalysis::eigen_decomposition(double A[3][3], double V[3][3], double d[3]) 
{
	int n = 3;
	double e[3];
	double da[3];
...
void tred2(double V[3][3], double d[3], double e[3]) 
{
	...
	for (i = n-1; i > 0; i--) {
		...
		scale = 0.0;
		h = 0.0;
		for (k = 0; k < i; k++) { scale = scale + std::fabs(d[k]); }
		if (scale == 0.0) {
			e[i] = d[i-1];
...
void tql2(double V[3][3], double d[3], double e[3]) 
{
	...
	for (l = 0; l < n; l++) {
		...
		tst1 = MAX_1(tst1, std::fabs(d[l]) + std::fabs(e[l]));
		m = l;
		while (m < n) {
