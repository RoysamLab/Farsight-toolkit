...
public:
  ...
  vtkstd::vector<vtkTree *> GetTraces() { return this->Traces; }
  ...
  void LoadTrees(const char *basefilename);
  int CreateTrees();
...
protected:
  ...
  bool PointInSoma(double point[3]);
...
private:
  ...
  vtkstd::vector<vtkTree *> Traces;
  vtkSmartPointer<vtkIdTypeArray> Orphans;
  //a list of trace lines that are rooted in the somas
  //the vtkIdType is the cell id of the trace line, and the double[3] is the
  //end point of the trace line that is NOT in the soma.
  vtkstd::list< vtkstd::pair<vtkIdType, double *> > TraceRoots;
