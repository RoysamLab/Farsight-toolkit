	...
	{		
		if (gTheVessels.m_apData[i])
		{
			...
			if(gTheVessels.m_apData[i]->GetLength() < 10)
				continue;
			//Then:
			//1- Process the Top of the vessel
			ProcessTip(gTheVessels.m_apData[i],Top,i+1);
			//2- Process the End of the vessel
...
bool CBranches::ProcessTip(CVessel* CurrentVessel, TopEnd Flag, int VessID)
{
	...
	//2. Calculate the enties of the rotation matrix
	...
	int threshold = 1;
	if(Flag == Top)
		br = DetectBranch(lng,threshold,VessID, OnTop);
	else
		br = DetectBranch(lng,threshold,VessID, OnEnd);
	if(br == false)
...
void CBranches::Calc3DRotMatrix(int d_x, int d_y, int d_z)
{
	int i, j, k;
...
bool CBranches::DetectBranch(int lng, int threshold, int SegID, TopEndMiddleNeither Flag)
{	
	...
	for(k=0;k<lng;k++)
	{
		...
		for(j=0;j<9;j++)
		{
			...
			if(tmp2[pnt[2]][pnt[1]][pnt[0]]!=0 && tmp2[pnt[2]][pnt[1]][pnt[0]]!=SegID)
			{
				pnt[1] -= 10;
				pnt[2] -= 10;
				int p0 = Rot_3D[0][0]*pnt[0] + Rot_3D[0][1]*pnt[1] + Rot_3D[0][2]*pnt[2];
				int p1 = Rot_3D[1][0]*pnt[0] + Rot_3D[1][1]*pnt[1] + Rot_3D[1][2]*pnt[2];
				int p2 = Rot_3D[2][0]*pnt[0] + Rot_3D[2][1]*pnt[1] + Rot_3D[2][2]*pnt[2];
				BrPnt = new CPoint(p0+TipPoint->m_iX, p1+TipPoint->m_iY, p2+TipPoint->m_iZ, TipPoint->m_iHDir, TipPoint->m_iVDir, TipPoint->m_iValue);
				NumberOfBranches++;
				hit = 1;
				break;
			}			
