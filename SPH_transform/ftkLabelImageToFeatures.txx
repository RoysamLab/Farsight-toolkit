...
{	
	...
	if(computationLevel >= 3)
	{
		...
		RunZernikeFilter();
		if(this->computeSurface)
			RunSphericalHarFilter();		
	}
...
::CalculateScanFeatures()
{
	...
	//boundaryPix.clear(); ///////////////////////////////////////////////Commented by Yanbin
	interiorPix.clear();
}
...
{
	return false;
	std::cout<<"Run Texture Features!"<<std::endl;
	if(!intensityImage || !labelImage) return false;
		...
		labelImage->GetLargestPossibleRegion().GetSize()[2]==1	)
  {
	...
	TLPixel currentLabel;
	#pragma omp parallel for             //////////////////////added by yanbin for parallel
	for (int lab=0; lab<(int)labels.size(); ++lab)
	{
		currentLabel = labels.at(lab);
		if ((int)currentLabel <= 0) continue;
...
{
	return;
	//std::cout<<"Run Surface Spherical Harmonic Coeff !"<<std::endl;
	if( !boundaryPix.size() || !labels.size() || !intensityImage)
	{
		std::cout<<"BoundaryPix OR label OR intensity image is not valid! "<<std::endl;
		return;
	}

	float * centroid;
	typename LabelImageType::IndexType point;
	typename LabelImageType::IndexType pointcoordin;
	TLPixel currentLabel;
	std::vector< std::vector< typename LabelImageType::IndexType > > boundaryPixCorrdinate;
	boundaryPixCorrdinate.resize(labels.size());

	for (int lab=0; lab<(int)labels.size(); ++lab)
	{
		currentLabel = labels.at(lab);
		if ((int)currentLabel <= 0) 
			continue;

		centroid = this->featureVals[currentLabel].Centroid;

		boundaryPixCorrdinate.at(LtoIMap[currentLabel]).resize( boundaryPix.at(LtoIMap[currentLabel]).size() );
		for (int i=0; i<(int)boundaryPix.at(LtoIMap[currentLabel]).size(); ++i)
		{
			point = boundaryPix.at(LtoIMap[currentLabel])[i];
			typename LabelImageType::IndexType pointcoordin;
			...
			for(unsigned int n = 0; n < VImageDimension; n++)
			{
				pointcoordin[n] =  point[n] - centroid[n];
			}
			boundaryPixCorrdinate.at(LtoIMap[currentLabel])[i] = pointcoordin;
		}
	}
...
//#endif
	//for (int lab=0; lab<(int)labels.size(); ++lab)
	//{
	//	std::vector<std::vector<double> > surfacedata;
	//	surfacedata.resize(boundaryPixCorrdinate[lab].size());
	//	for (int j=0; j<boundaryPixCorrdinate[lab].size(); ++j)
	//	{
	//		pointcoordin = boundaryPixCorrdinate[lab][j];
	//		surfacedata[j].push_back(pointcoordin[0]);
	//		surfacedata[j].push_back(pointcoordin[1]);
	//		surfacedata[j].push_back(pointcoordin[2]);
	//	}
	//	SPH_Trasform* sph_trans = new SPH_Trasform();
	//	bool ini = sph_trans->Initialize(surfacedata, L);
	//	sph_trans->set_small();
	//	featureVals[lab].ScalarFeatures[IntrinsicFeatures::CONVEXITY] = sph_trans->Get_convexity( );
	//	if( !ini)
	//	{
	//		//std::cout<<" label "<<lab<<" failed in initialization , no integrated surface ! "<<std::endl;
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_1] = float( 0 );
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_2] = float( 0 );
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_3] = float( 0 );
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_4] = float( 0 );
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_5] = float( 0 );
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_6] = float( 0 );
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_7] = float( 0 );
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_8] = float( 0 );
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_9] = float( 0 );
	//		featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_10] = float( 0 );
	//	}
	//	else
	//	{
	//		bool tran = sph_trans->Transform();
	//		if(!tran)
	//		{
	//			//std::cout<<" label "<<lab<<" failed in data fitting , too less surface point ! "<<std::endl;
	//			featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_1] = float( 0 );
	//			featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_2] = float( 0 );
	//			featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_3] = float( 0 );
	//			featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_4] = float( 0 );
	...
	//			double sum = 0.0;
	//			for(int k = 0; k < 5; k++)
	//				sum += sph_trans->degreecoeff(k);
	//			featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_1] = float( sph_trans->degreecoeff(0) );
	//			featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_2] = float( sph_trans->degreecoeff(1) );
	//			featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_3] = float( sph_trans->degreecoeff(2) );
	//			featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_4] = float( sph_trans->degreecoeff(3) );
	//			if(sum < 1e-4)
	//			{
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_5] = float( 0 );
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_6] = float( 0 );
	...
	//			else
	//			{
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_5] = float( sph_trans->degreecoeff(0) / sum );
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_6] = float( sph_trans->degreecoeff(1) / sum );
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_7] = float( sph_trans->degreecoeff(2) / sum );
	//			}
	//			if(sph_trans->degreecoeff(0) < 1e-4)
	//			{
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_8] = float( 0 );
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_9] = float( 0 );
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_10] = float( 0 );
	...
	//			else
	//			{
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_8] = float( sph_trans->degreecoeff(1) / sph_trans->degreecoeff(0) );
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_9] = float( sph_trans->degreecoeff(2) / sph_trans->degreecoeff(0) );
	//				featureVals[lab].ScalarFeatures[IntrinsicFeatures::SPH_10] = float( sph_trans->degreecoeff(3) / sph_trans->degreecoeff(0) );
	//			}
	//		}
	//	}
	//}
	
	//*************************************output surface coordinate file*********************************/
	//const char* filename = "suarface_coordinate400_1000";
	...
	//	fprintf(fp,"%d", i);
	//	fprintf(fp,"\n");
	//	centroid = this->featurevals[i].centroid;
	//	fprintf(fp,"%f",(double)centroid[0]);
	//	fprintf(fp,"\t");
	//	fprintf(fp,"%f",(double)centroid[1]);
	//	fprintf(fp,"\t");
	//	fprintf(fp,"%f",(double)centroid[2]);
	//	fprintf(fp,"\n");
	//	std::cout<<"label is "<<i<<std::endl;
	//	std::cout<<"size is "<<boundarypixcorrdinate[i].size()<<std::endl;
	//	for (int j=0; j<boundarypixcorrdinate[i].size(); ++j)
	//	{
	//		pointcoordin = boundarypixcorrdinate[i][j];
	//		//std::cout<<"x is "<<(double)pointcoordin[0]<<" y is "<<(double)pointcoordin[1]<<" z is "<<(double)pointcoordin[2]<<std::endl;
	//		fprintf(fp,"%f",(double)pointcoordin[0]);
	//		fprintf(fp,"\t");
	//		fprintf(fp,"%f",(double)pointcoordin[1]);
	//		fprintf(fp,"\t");
	//		fprintf(fp,"%f",(double)pointcoordin[2]);
	//		fprintf(fp,"\n");
	//	}
	//}
	//fclose(fp);
