...
// The first line contains the value of Class_Membership
// The second line should contain the other variable names
// Values start from the third row
// Values are delimited by comma. Variable names and values for ID,X,Y,Z 
// are required to be given in addition to the Class_Membership value 
...
// Neuron will be added as a Class_Membership
// Use this program to read Tara's text input
...
void writeXML(vector<vector<string>> values, char* fileName) {	
	...
	// The first line contains the value of Class_Membership
	// The second line should contain the variable names
	// Values start from the third row
	for (i=2;i<values.size();i++) {
		...
		for (j=0;j<values[1].size();j++) 
			element= element + values[1][j] + "=\"" + values[i][j]+"\" ";
		//Now add Class_Membership variable and its value too
		//cout<<"hey"<<endl;
		element=element + " Class_Membership =\""+values[0][0]+ "\" ";
		element=element + ">";
...
int main(int argc, char* argv[]) {
	...
	variableNames=values[1];
	variableNames.push_back("Class_Membership");
