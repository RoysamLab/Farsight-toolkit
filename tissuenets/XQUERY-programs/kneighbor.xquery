	...
	return
     for $otherObject in doc($nodes2)/objects/Nuclear_Features
		...
		let $dz := $otherObject/@Z -  $currObject/@Z 
                ...
                (:For synaptic x=0.047   y=0.047   z=0.07  :) 
               	(: let $dxx := $dx * $dx * 0.047 * 0.047   :)
               	(: let $dyy := $dy * $dy * 0.047 * 0.047   :)
               	(: let $dzz := $dz * $dz * 0.07 * 0.07     :)
          ...
          element distances {
	          ...
	          attribute targets {data(doc($nodes2)/objects/@class)},
		       ...
		       return
		        	...
		        	element X {0.36 * data($i[$j]/X)},
		        	element Y {0.36 * data($i[$j]/Y)},
		        	element Z {1.5  * data($i[$j]/Z)},
