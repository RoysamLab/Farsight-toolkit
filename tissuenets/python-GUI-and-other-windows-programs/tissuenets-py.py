...
def ConstructRadioButtons(self, rowno, columnno, itemList):
      RBFrame = Frame(self.dlgNodeFrame,width=10, height=10, bd=1).grid(row=rowno,column=columnno)
      #.grid(row=row,column=column, sticky=W)
      for fobject in itemList.split(" "):
               self.objectTypes=Radiobutton(RBFrame,
                     text=fobject,
                     variable=self.dlgNodeFrame.txtObjectType,
                     value=fobject
                     ).grid(row=self.row,column=0,sticky=W)
               self.row+=1
      #RBFrame.pack(side=BOTTOM)         
      #self.dlgNodeFrame.pack()
      #return RBFrame        
 
      #self.txtObjectType=RBFrame.txtObjectType         
...
dcutOff=""
###########################################
class dlgConvertFileToXML(Toplevel):
   def __init__(self, master):

      master.title('Convert a Text File into Proper XML File')
      ...
      Label(dlgNodeFrame,
            text = "Input: Filename of the text file to be converted into XML :"
      ...
      # Ask for Object type
      Label(dlgNodeFrame,
            text = " Input2: Text File Format :"
            ).grid(row=3, column=0, sticky =W)
      self.txtObjectType=dlgNodeFrame.txtObjectType = StringVar()
      
      allObjects = ["FARSIGHT Text File", "EXCEL generated Text File"]
   ...
   def cmdConvert(self):
      ...
      prgName=""
      # Get the File type from the radio button group
      fileType = ""
      fileType = self.txtObjectType.get()
      fileName1=self.txtNodeInFileName.get()

      if (fileType != "") and (fileName1 != ""):
            print "Converting into XML ..."
            if (fileType == "FARSIGHT Text File"):
                  prgName ="readtextfile "
            else:
                  prgName ="readtextfiletara "            
            osCommand = prgName+fileName1            
            os.system(osCommand)
            showinfo('Warning!', 'Conversion is done. Check command line messages for any errors')
      else:
            showerror('Error!', 'Input is Incomplete')
...
class dlgCreateNode(Toplevel):
   def __init__(self, master):
      ...
      self.master=master
      dlgNodeFrame = Frame(self,width=50, height=60, bd=1)
      self.dlgNodeFrame = dlgNodeFrame
      dlgNodeFrame.pack()
      #call grabset to make dialog modal= Only 1 dialog can appear on the screen
      #We don't want to confuse users
      self.grab_set()
      ...
      self.listbox = Listbox(dlgNodeFrame,selectmode=SINGLE)
      #listbox.pack()
      ...
      self.listbox.grid(row=rowno+1,column=0)

      #allObjects = ["Astrocytes", "Microglia", "Neurons","Endothelials","Electrode", 
      #              "Npresynaptic", "Mpostsynaptic"]
      #dlgNodeFrame.txtObjectType.set(2)
      #column = 2
      #row=6
      #for fobject in allObjects:
      ...
      emptyRows(4,rowno,dlgNodeFrame)
   ...
   def cmdBrowse2(self):
      ...
      if fileName2:
            ...
            data = child.read()
            allObjects=data
            self.dlgNodeFrame.txtObjectType.set(2)
            self.column = 2
            self.row=6

            self.listbox.delete(0, END)
            for item in allObjects.split(" "):
                  self.listbox.insert(END, item)
            #self.listbox.grid(row=rowno+1,column=0)

            #ConstructRadioButtons(self, self.row, self.column, allObjects)
            err = child.close()
            if err:
   ...
   def cmdBrowse3(self):
      ...
      if fileName2:
            ...
            self.row=6
            #self.objectTypes.
            for fobject in allObjects.split(" "):
               self.objectTypes=Radiobutton(self.dlgNodeFrame,
                     text=fobject,
                     variable=self.dlgNodeFrame.txtObjectType,
                     value=fobject
                     ).grid(row=self.row,column=0,sticky=W)
   ...
   def cmdCreateNodes(self):
      ...
      # Get the object types from the listbox
      #objType = ""
      objType = self.txtObjectType.get()
      objType = self.listbox.curselection()
      #
      ...
      # get the line's text
      #
      objType = self.listbox.get(index)
...
class MainMenu:
   ...
   def convertFileToXML(self):
     crtNodeObj=dlgConvertFileToXML(root)
   ...
   def __init__(self,win):
      ...
      menuCreateNetwork.add_command(label='Step 0: Convert a Text File into Proper XML File  ...', command=self.convertFileToXML, underline=0)
