...
map<char,int> setColorCode() {
	...
	channelColorsTable['N']=4; //blue Astrocytes
							//5 if the vertex is a member of out-pyramidal region
...
/**************************************************************************************/
// A helper-function that is used for computing averages for pyramidal region          /
...
// Note also that this function can be used to produce averages too for pyramidals     /
/**************************************************************************************/
void pyramidal(vtkGraph* g, float cutoff) {
...
*/
  ...
  if (myfile.is_open())
  {
  //cout<<"Average Edge Lengths for pyramidal Cell Layer:"<<endl;
  while (vertices->HasNext())
	...
	if (neighborCount > 0) {
		...
		// Out pyramidal - 5 defines the color
...
VTK_CREATE(vtkPoints,pts);
	...
	//if (e > 0) avg = avg/e;  //Average edge length for pyramidal region computation
...
*/
 ...
 //lut->SetTableValue(5, 0.5765, 0.4392, 0.8588, 1); //medium-purple for pyramidal
...
int main(int argc, char *argv[])
{
	...
	readXML(argv[1],distance,g);
 	...
 	if (stdMst == 2) pyramidal(g,cutoff);
	if (stdMst == 1) {
