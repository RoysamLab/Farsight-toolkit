...
//-----------------------------------------------------------------------------
void vtkPVPythonInterpretor::LoadInput()
{
...
void vtkPVPythonInterpretor::InitializeInternal()
{
  ...
  if (this->CaptureStreams)
    ...
    vtkPVPythonInterpretorWrapper* wrapperIn = vtkWrapInterpretor(this);
    wrapperIn->DumpToError = false;

    // Redirect Python's stdout, stderr, and stdin
    ...
    PySys_SetObject(const_cast<char*>("stdin"),
      reinterpret_cast<PyObject*>(wrapperIn));
...
//-----------------------------------------------------------------------------
bool vtkPVPythonInterpretor::Push(const char* const code)
{
  return true;
}
