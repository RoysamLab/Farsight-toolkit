...
void ftkMainDarpaSegment::runSpliting()
{
	...
	if( !_TRI_Image.empty() )
	{
		...
		splitStore< rawImageType_8bit >( ImageMontage_TRI, _TRI_Image );
	}
	if( !_GFP_Image.empty() )
	{
...
void ftkMainDarpaSegment::runInterestPoints(  )
{
...
// 				std::cout << std::endl << "REGION_MON: " << regionMontage_inside;
				...
				if( !_TRI_Image.empty( ) && !_DAP_Image.empty() )
				{
					...
					std::string _TRI_ImageNoPath = _TRI_Image.substr(foundGFP+1);
				...
				if( !_Dist_Map_Image.empty( ) )
				{
					...
					std::string _Dist_Map_ImageNoPath = _Dist_Map_Image.substr(foundGFP+1);
					// !!! this has to be nrrd is just to test
					std::string tempDist_Map = _outPathTemp+"/"+_Dist_Map_ImageNoPath+"_"+xStr+"_"+yStr+"_"+zStr+".nrrd";
					imageLocalDist_Map = readImage< rawImageType_8bit >(tempDist_Map.c_str());
				}
				}
...
// 				RunSegmentation(regionLocal_inside, Images_Tiles, Label_Tiles, Table_Tiles, Centroids_Tiles);
				...
				AT->LoadCurvImage(Images_Tiles[1], 0);
				...
				AT->ComputeAstroFeaturesPipeline(featureVectorFileName, IDImageFileName, 0, std::string("LOG"));
...
void ftkMainDarpaAstroTrace::computeSplitConst( )
{
