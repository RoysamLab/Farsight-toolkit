  ...
  for(int counter=0; counter<num_t; counter++)
	...
	/*else
	{
		//for(int c=1;c<=2; c++)
		{
			for(int t = 0; t< num_t; t++)
			{
				sprintf(tracks_cache,"%s/%s",CACHE_PREFIX,file_name[(num_files*2)+(c-1)+(num_t*t)]);
				segmented[t][c-1] = readImage<LabelImageType>(tracks_cache);
				getFeatureVectorsFarsight(segmented[t][c-1],images[t][c-1],fvector[t][c-1],t,c);
			}
		}
	...
	sprintf(vessel_trace_cache,"%s/%s/vessel_trace_%s_w%d.tif",CACHE_PREFIX,dataset.c_str(),dataset.c_str(),c);
	if(!file_exists(vessel_trace_cache))
	{
		InputImageType::Pointer vessel_max_binarized;
		char vessel_max_cache[1024];
		sprintf(vessel_max_cache,"%s/%s/vessel_max_%s_w%d.tif",CACHE_PREFIX,dataset.c_str(),dataset.c_str(),c);
		if(!file_exists(vessel_max_cache))
		{
			InputImageType::Pointer binary;
			for(int t = 0; t< num_t; t++)
			{
				sprintf(vessel_cache,"%s/%s/vessel_binarized_%s_w%d_t%d.tif",CACHE_PREFIX,dataset.c_str(),dataset.c_str(),c,files[t*num_channels].time_point);
				...
				{
					binary = readImage<InputImageType>(vessel_cache);
				}
				if(t==0)
				{
		...
		{
			vessel_max_binarized = readImage<InputImageType>(vessel_max_cache);
		}
		...
		writeImage<InputImageType>(vessel_trace,vessel_trace_cache);
	}
	else
	{
		vessel_trace = readImage<InputImageType>(vessel_trace_cache);
	}
	...
	for(int c = 1; c<=2; c++)
	{
		...
		AnalyzeDCContact(segmented,tfs,3,num_t);
		char tracks_summary[1024];
		printf("tfs.size = %d\n",tfs.size());
		sprintf(tracks_summary,"%s/%s/whole_track_features_%s_w%d.txt",CACHE_PREFIX,dataset.c_str(),dataset.c_str(),c);
		FILE*fp1 = fopen(tracks_summary,"w");
		char nucleus_summary[1024];
		sprintf(nucleus_summary,"%s/%s/track_point_features_%s_w%d.txt",CACHE_PREFIX,dataset.c_str(),dataset.c_str(),c);
