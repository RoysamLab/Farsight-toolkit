...
int main( int argc, char * argv[] )
{
  if( argc < 4 )
    ...
    std::cerr << argv[0] 
      << "  inputImageFile  outputImageFile factor_XY" 
      << std::endl; 
    return EXIT_FAILURE;
    }
...
// Software Guide : BeginCodeSnippet
  /*
  const double space_X = atof( argv[3] );
  const double space_Y = atof( argv[4] );
  const double space_Z = atof( argv[5] );
  */
  const double factorX = atof( argv[3] );
  const double factorY = atof( argv[3] );
