...
if platform.system() == 'Windows':
  ...
  regp = './register_pair_16'
  regj = './register_joint_16'
  moim = './mosaic_images_16'
...
def register(argv):
  ...
  for line in f:
    ...
    subprocess_command_list.append(regp+' '+image_dir + from_image+' ' + image_dir +to_image+' -multiThread 0 ' +' -smooth 0 '+' -remove_2d'+' -gdbicp '+image_dir)
 #subprocess_command_list.append(regp)
  ...
  for subprocess_command in subprocess_command_list:
    ...
    #print lista
    #print lista[3]
    memoryAvilable = 1 # for now FIXME
    if int(lista[3]) < 150000000:
      memoryAvilable = 1
    print memoryAvilable
    print lista[3]+str(memoryAvilable)  

    #while (threads_launched >= max(1,multiprocessing.cpu_count() - 5) ) | (memoryAvilable == 0): # at least one thread, but no more than 3/4 of the machine # for - 10 now FIXME

   #//---------------------------------------------------------------------------------------------------------------------
   # MAIN PARAMETER, NUMBER OF SYMULTANEOUS REGISTRATION AT THE SAME TIME
   #//---------------------------------------------------------------------------------------------------------------------
    while (threads_launched >= max(1, multiprocessing.cpu_count() - 10 ) ) | (memoryAvilable == 0): # at least one thread, but no more than 3/4 of the machine # for - 10 now FIXME
