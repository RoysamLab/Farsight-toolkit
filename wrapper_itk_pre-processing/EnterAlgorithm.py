...
#pdb.set_trace()
	...
	def GetCasterWidget(self):
		self.NextRow()
		wx.StaticText(self.panel, -1, pos = (20, self.tempRow), label = "Don't use Caster (rescale intensities)?")
		self.notUseCasterCheckBox = wx.CheckBox(self.panel, 1, pos = (200, self.tempRow))
...
#pdb.set_trace
	...
	def AlgorithmSubmit(self, event):		
		...
		thisAlgorithm.SetUseCaster(
				not self.notUseCasterCheckBox.GetValue())
...
#pdb.set_trace()
		...
		xmlObject.AddAlgorithm(thisAlgorithm)
...
#pdb.set_trace()
		if algorithm:	
			...
			if not algorithm.GetUseCaster():
				self.notUseCasterCheckBox.SetValue(wx.CHK_CHECKED)
			else:
				self.notUseCasterCheckBox.SetValue(wx.CHK_UNCHECKED)
