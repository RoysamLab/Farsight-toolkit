...
#ifndef __BLOCK_B_H__
#define __BLOCK_B_H__
...
template <class Type> class Block_B
{
public:
	...
	Block_B(int size, void (*err_function)(char *) = NULL) { first = last = NULL; block_size = size; error_function = err_function; }

	/* Destructor. Deallocates all items added so far */
	~Block_B() { while (first) { block *next = first -> next; delete first; first = next; } }
...
template <class Type> class DBlock_B
{
public:
	/* Constructor. Arguments are the block size and
	   (optionally) the pointer to the function which
	   will be called if allocation failed; the message
	   passed to this function is "Not enough memory!" */
	DBlock_B(int size, void (*err_function)(const char *) = NULL) { first = NULL; first_free = NULL; block_size = size; error_function = err_function; }

	/* Destructor. Deallocates all items added so far */
	~DBlock_B() { while (first) { block *next = first -> next; delete first; first = next; } }
