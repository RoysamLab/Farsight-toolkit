...
template <typename captype, typename tcaptype, typename flowtype> class Graph_B
{
public:
	typedef enum
	{
	...
	flowtype maxflow(bool reuse_trees = false, Block_B<node_id>* changed_list = NULL);
...
flowtype Graph_B<captype,tcaptype,flowtype>::maxflow(bool reuse_trees, Block_B<node_id>* _changed_list)
{
	...
	if (!nodeptr_block)
	{
		nodeptr_block = new DBlock_B<nodeptr>(NODEPTR_BLOCK_SIZE, error_function);
	}
