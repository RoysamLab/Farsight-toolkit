...
GCoptimization::EnergyType GCoptimization::giveDataEnergyArray()
{
	...
	{
		float yyy = m_datacost(i,m_labeling[i]);
		eng = eng + yyy;
...
GCoptimization::EnergyType GCoptimization::giveSmoothEnergy_NG_FnPix()
{
	...
	for ( i = 0; i < m_num_pixels; i++ )
		if ( !m_neighbors[i].isEmpty() )
		{
			...
			//int ll = m_neighbors[i].getBlockSize();
			//for(int kk=0; kk<ll; kk++) //modified by yousef
			{
	...
	if ( size > 0 ) 
	{
		...
		if ( m_grid_graph )
		{
			if ( m_smoothType == ARRAY )
			{
				...
				std::cerr<<"GraphCut:AlphaExpansion:smmoth_type->Smooth type FUNCTION_PIX not supported"<<std::endl;
                //mexErrMsgIdAndTxt("GraphCut:AlphaExpansion:smmoth_type","Smooth type FUNCTION_PIX not supported");
                /* BAGON REPLACED 
			...
			{
				std::cerr<<"GraphCut:AlphaExpansion:smmoth_type->Smooth type not supported"<<std::endl;
                //mexErrMsgIdAndTxt("GraphCut:AlphaExpansion:smmoth_type","Smooth type not supported");
                /* BAGON REPLACED 
				printf("NOT SUPPORTED YET,exiting!");
				exit(0); */
		...
		{
			std::cerr<<"GraphCut:AlphaExpansion:m_grid_graph->not m_grid_graph - unsupported"<<std::endl;
            //mexErrMsgIdAndTxt("GraphCut:AlphaExpansion:m_grid_graph","not m_grid_graph - unsupported");
            /* BAGON REPLACED 
			printf("NOT SUPPORTED YET,exiting!");
			exit(0); */
...
{ 
   if  (error_condition) 
   {
	   std::cerr<<"GraphCut:GCoptimization:errorFn->erros"<<std::endl;
       //mexErrMsgIdAndTxt("GraphCut:GCoptimization:errorFn","error %s",message);
       /* BAGON REPLACED
      mexPrintf("\n %s \n", message); // BAGON changed printf -> mexPrintf
      exit(1); */
    }
}
