...
Graph::node_id Graph::add_node()
{
	...
	if (!node_block_first || node_block_first->current+1 > &node_block_first->nodes[NODE_BLOCK_SIZE-1])
	{
		...
		if (!node_block_first)
			std::cerr << "GraphCut:Graph:add_node->not enough mem" << std::endl;
            //mexErrMsgIdAndTxt("GraphCut:Graph:add_node","not enough mem"); // BAGON REPLACED { if (error_function) (*error_function)("Not enough memory!"); exit(1); }
...
void Graph::prepare_graph()
{
	...
	for (nb=node_block_first; nb; nb=nb->next)
	{
		for (i=&nb->nodes[0]; i<nb->current; i++)
		{
			...
			{
				if (k > ARC_BLOCK_SIZE)
					std::cerr<<"GraphCut:Graph:prepare_graph-># of arcs per node exceeds block size"<<std::endl;
                    //mexErrMsgIdAndTxt("GraphCut:Graph:prepare_graph","# of arcs per node exceeds block size (out)"); // BAGON REPLACED{ if (error_function) (*error_function)("# of arcs per node exceeds block size!"); exit(1); }
				...
				if (ab_for == NULL)
				{
					...
					char *ptr = new char[sizeof(arc_for_block)+1];
					if (!ptr) 
						std::cerr<<"GraphCut:Graph:prepare_graph->not enough mem"<<std::endl;
                        //mexErrMsgIdAndTxt("GraphCut:Graph:prepare_graph","not enough mem"); // BAGON REPLACED{ if (error_function) (*error_function)("Not enough memory!"); exit(1); }
					if ((POINTER_CAST)ptr & 1) arc_for_block_first = (arc_for_block *) (ptr + 1);
					else              arc_for_block_first = (arc_for_block *) ptr;
					arc_for_block_first -> start = ptr;
					arc_for_block_first -> current = & ( arc_for_block_first -> arcs_for[0] );
			...
			{
				if (k > ARC_BLOCK_SIZE)
					std::cerr<<"GraphCut:Graph:prepare_graph-># of arcs per node exceeds block size (in)"<<std::endl; // BAGON REPLACED { if (error_function) (*error_function)("# of arcs per node exceeds block size!"); exit(1); }
                    //mexErrMsgIdAndTxt("GraphCut:Graph:prepare_graph","# of arcs per node exceeds block size (in)"); // BAGON REPLACED { if (error_function) (*error_function)("# of arcs per node exceeds block size!"); exit(1); }
