	...
	Type *New(int num = 1)
	{
		...
		if (!last || last->current + num > last->last)
		{
			...
			else
			{
				...
				if (!next)  
					std::cerr<<"GraphCut:Block:New->Not enough mem"<<std::endl;
                    //mexErrMsgIdAndTxt("GraphCut:Block:New","Not enough mem"); // BAGON REPLACED{ if (error_function) (*error_function)("Not enough memory!"); exit(1); }
...
public:
	...
	Type *New()
	{
		...
		if (!first_free)
		{
			...
			if (!first)
				std::cerr<<"GraphCut:DBlock:New->Not enough mem"<<std::endl;
                //mexErrMsgIdAndTxt("GraphCut:DBlock:New","not enough mem"); // BAGON REPLACED { if (error_function) (*error_function)("Not enough memory!"); exit(1); }
