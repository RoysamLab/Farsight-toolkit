...
seg_graphs::RAGraph seg_graphs::BuildRAG(unsigned short id)
{
	Initialize(id);
	...
	while (counter != num_vertices(this->RAG))
	{
		...
		if(RAG_cells.size()>0)
		{
			...
			{	
				...
				bool bRet;
				Edge e;
				tie(e,bRet) = edge(root,tail,this->RAG);
				if(!bRet)
				{
...
void seg_graphs::Initialize(unsigned short id)
{		
...
// NEED TO TEMPLATE
///////////////////////////////////////////////////////////////////////////////////////////////////////
std::string seg_graphs::convert2string(unsigned short id)
{
	std::stringstream out;
	out << id;
	std::string s = out.str();
...
int seg_graphs::GetNodeIndex(unsigned short id,seg_graphs::RAGraph graph1)
{
	...
	int index;
	int flag = -1;
	property_map<seg_graphs::RAGraph, vertex_name_t>::type nodes = get(vertex_name, graph1);
...
void seg_graphs::setFeats(std::vector<FeaturesType> allFeat,std::vector< unsigned short > labelIndex)
{
...
seg_graphs::MTreeType seg_graphs::BuildMergeTreeDcon(seg_graphs::RAGraph R1, unsigned short id,std::vector< std::set<int> > hypothesis)
{
	//Create the Merge Tree
	seg_graphs::MTreeType mTree; 
	std::set<int> currRPS;
	std::set<int> nextRPS;	
	...
	seg_graphs::AdjVertIt avi, avinext, av_end;
	...
	property_map<seg_graphs::RAGraph,vertex_name_t>::type nodes = get(vertex_name, R1);
	...
	while (counter != num_vertices(mTree))
	{
		...
		seg_graphs::node_mt V2 = vertex(counter,mTree);
		...
		{	
			...
			seg_graphs::node v = vertex(vertex_index,R1);
			tie(avi, av_end)=adjacent_vertices(v,R1); 
			
			for (avi=avi; avi < av_end ; ++avi)
			{
				nextRPS = currRPS;	   
				seg_graphs::node X = *avi;
