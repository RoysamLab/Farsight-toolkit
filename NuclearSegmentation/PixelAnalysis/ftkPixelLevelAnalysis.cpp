...
bool ftk::PixelLevelAnalysis::RunAnalysis4(){
	...
	pixBufRoiClean.GoToBegin(); pixBufTargDial.GoToBegin();
	roi_count = 0; target_count = 0;
	for( ; !pixBufRoiClean.IsAtEnd(); ++pixBufRoiClean, ++pixBufTargDial )
	{
			if( pixBufRoiClean.Get() && pixBufTargDial.Get() ) ++target_count;
			if( pixBufRoiClean.Get() ) ++roi_count;
	}
